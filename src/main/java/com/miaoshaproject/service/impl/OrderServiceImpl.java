...
//            @Override
//            public void afterCommit() { //此方法会在最近的一个transaction提交之后执行，最近的就是创建订单这个transaction
                //异步更新库存
//                Boolean mqResult = itemService.asyncDecreaseStock(itemId,amount);
//                if(!mqResult){  //如此也会存在一个问题，即如果mqResult发送失败，就没有机会再回滚库存了
//                    //库存回滚
...
//        });
        ...
        //返回前端
        return orderModel;
        //就算以上操作都成功了，但最后提交操作的时候如果因为网络等原因导致异常的话，asyncDecreaseStock也不会起作用，可以考虑用springboot提供的
        //
    }

    //REQUIRES_NEW表示只要当前事务只要成功了就应该提交，不需管其他业务的逻辑是否成功，也就是当前生成的序列号不可再用了
    ...
    String generatorOrderNo(){
        ...
        //获取当前sequence
        int sequence = 0;
//        SequenceDO sequenceDO = sequenceDOMapper.selectByPrimaryKey("order_info"); //getSequenceByName
